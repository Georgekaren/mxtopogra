...
Menus = function(editorUi)
{
	...
	data="查看属性+m1&查看下级-my2+m2";
	//获取右键菜单 关联设备信息数据
	/*$.post($("#path").val()+"/GetConnIpServlet",function(text){
		text="127.0.0.1/1222-my+m1&127.0.0.2/1222-my2+m2";
		if(text!=null && text!=""){
			data = text;
		}
	});
...
Menus.prototype.init = function()
{
	...
	//右键关联设备信息{二级，三级...菜单}
	this.put('connip', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		var idx=0;
		var sizes = data.split("&");
		for(var i = 0;i<sizes.length;i++){
			if(sizes[i]!="" && sizes[i]!=null){
				idx = sizes[i].indexOf("+");
				if(idx>0){
					this.valueChange(menu, sizes[i].substr(0,idx),sizes[i].substr(sizes[i].indexOf("+")+1), null, parent);
				}else{
					this.valueChange(menu, sizes[i],"", null, parent);
				}
			}
		}
		menu.addSeparator(parent);
	...
	this.put('text', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		...
		this.addMenuItems(menu, ['-', 'bold', 'italic','-'], parent);
	...
	{
		this.addMenuItems(menu, ['actualSize','refreshView'], parent);
		menu.addSeparator();
	...
	{
		//this.addMenuItems(menu, ['save', '-', 'import', 'editFile', '-', 'pageSetup', 'print'], parent);
		this.addMenuItems(menu, ['pageSetup'], parent);
	})));
	this.put('edit', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['undo', 'redo', '-', 'cut', 'copy', 'paste', 'delete', '-', 'duplicate', '-',
		                        /* 'editLink', 'openLink', '-', 取消编辑连接和打开连接*/
	...
	/*this.put('initNext', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['initNextRes'], parent);
        var graph = this.editorUi.editor.graph;
		
		graph.getModel().beginUpdate();
		try
		{
			var cels = graph.getSelectionCell();//获取当前选中的cell对象
			var vl=cels.getValue();
			cels.setValue("111");
			cels.setRemark(values);
			//cels.setFid(values);
			graph.refresh();//刷新整个区域
		}
		finally
		{
			graph.getModel().endUpdate();
		}
		//this.addMenuItems(menu, ['initNext'], parent);
		//menu.addSeparator();
...
Menus.prototype.valueChange = function(menu, label, values, sprite, parent)
{
	return menu.addItem(label, null, mxUtils.bind(this, function()
	{
		var graph = this.editorUi.editor.graph;
		...
		{
			var cels = graph.getSelectionCell();//获取当前选中的cell对象
			cels.setValue(label.substr(label.indexOf("—")+1)+"\n通讯:"+label.substr(0,label.indexOf("—")).replace("/","\n业务:"));
			cels.setRemark(values);
			//cels.setFid(values);
			graph.refresh();//刷新整个区域
		}
...
Menus.prototype.createPopupMenu = function(menu, cell, evt)
{
	...
	{		
		this.addMenuItems(menu, ['toFront', 'toBack','-','layout','refreshView']);
		this.addSubmenu('linewidth', menu);
		if (graph.getSelectionCount() == 1){//如果选中的数量只有一个 
			var cel = graph.getSelectionCell();
			//graph.setCellStyles(mxConstants.GUIDE_COLOR,"#FFFFFF");
			//cel.style=cel.style+";color;color=#FFFFFF;";
			if(!cel.isEdge()){//如果选中的不是边 则增加关联ip菜单
				this.addMenuItems(menu, ['initNext','initResProperties','initSubRes','invisibleSub','invisibleOrther','viewSuperRoute']);
				//this.addSubmenu('initNext', menu);
			}else{
				this.addMenuItems(menu, ['viewLink']);
	...
	{
		this.addMenuItems(menu, ['-', 'selectVertices', 'selectEdges', '-', 'selectAll','refreshView']);
	}
};
